...
def test_do_hcl_body_empty_attribute_value():
    a = Attribute("website", [dict(name="", index="index.html")])
    assert (
        do_hcl_body(a)
        == """    website {
        index = "index.html"
...
def test_do_hcl_body_empty_list_attribute_value():
    a = Attribute("website", [dict(name="", index="index.html", ref=[])])
    assert (
        do_hcl_body(a)
        == """    website {
        index = "index.html"
...
def test_normalize_identity():
    ...
    assert normalize_identity("X_AccessS3Bucket,.-ami") == "X_AccessS3Bucket---ami"
