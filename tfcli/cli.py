...
@click.option(
    "--out-file",
    "-o",
    help="output path for extracted resource state, default will be <resource>.tfstate",
    required=False,
)
@click.option(
    "--state-file",
    ...
    help="original terraform state file",
)
...
def state_extract(ctx: click.Context, out_file: str, state_file, resources: list):
    ...
    logger.info(
        "extract from state file:{}, resources:{} to output:{}".format(
    ...
    try:
        ...
        if len(kept) == 0:
            logger.warning("No resource state is found for {}".format(resources))
        else:
            if dry_run:
                click.echo("\n".join(kept.keys()))
            else:
...
@click.option(
    "--out-file",
    "-o",
    help="output path for extracted resource state, default will be terraform.tfstate or <resource>.tfstate",
    required=False,
)
...
def migrate(ctx: click.Context, files):
    ...
    for tf in files:
        ...
        try:
            ...
            if proc.returncode != 0:
                logger.warning(
                    "{} is not valid. Error:{}".format(tf, o.decode() + e.decode())
                )
        finally:
            shutil.rmtree(td)
...
def sync(ctx: click.Context, types, output):
    ...
    click.echo(
        "sync {} to {}".format(",".join([_.__name__ for _ in flattened]), output)
    )
    for r in flattened:
